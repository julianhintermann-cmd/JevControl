...
- **Strukturierte Steuerung vor Simulation:**
  - Browser-DOM über eine eigene Erweiterung für Chrome, Edge, Firefox und Zen (Native Messaging),
...
3. Optional: die **Kairo-Browser-Erweiterung** laden – in Chrome/Edge entpackt aus dem Installationsordner, in
   Firefox/Zen die Datei `Kairo-Firefox.xpi` (siehe [docs/INSTALLATION.md](docs/INSTALLATION.md#browser-erweiterung)).
...
| `extension/` | Browser-Erweiterung für Chrome, Edge, Firefox und Zen (Manifest V3) |
...
**Automatisiert verifiziert** (Stand CI #14)
- Build aller Projekte; 106 Unit-Tests (Agent-Schleife, Jev-/OpenRouter-Protokoll, Planer, Sicherheit,
  Datei-Leser, Browser-Erkennung).
- Browser-Erweiterung: 45 Tests in echtem Chromium, inklusive Native Messaging; die DOM-Tests zusätzlich in
  Firefox (Gecko); Mozillas Prüfregeln (`web-ext lint`) ohne Fehler.
- Windows-Integration (27 Tests) mit echten Fenstern und echter Eingabe:
  ...
  - Native-Messaging-Relay mit dem echten `Kairo.BrowserHost.exe`, für Chrome/Edge- und Firefox/Zen-Aufrufe.
- End-to-End (8 Tests ohne KI-Modelle). Hier ersetzen ein skriptgesteuerter Planer und ein lexikalisches
  ...
  - **Firefox mit der echten Kairo-Erweiterung (DOM-Pfad):** Firefox 156 lädt die XPI als temporäres Add-on,
    der Host wird über den Mozilla-Registry-Eintrag gestartet, das Formular wird über den DOM-Pfad ausgefüllt
    (8 von 8 Eingaben bestätigt).
- Live mit echtem OpenRouter-Schlüssel (nur mit Repository-Secret `OPENROUTER_API_KEY`): Jev antwortet in rund
  150–200 ms; das Planungsmodell füllt das native Testformular in einer Runde korrekt aus (10,5 s, 0,015 USD).
...
- Installer:
  ...
  - Die stille Installation wird geprüft: Dateien, Startmenü- und Desktopverknüpfung, Autostart,
    Native-Messaging-Registrierung für Chrome, Edge und Firefox/Zen, `Kairo-Firefox.xpi`.
  - `Kairo.exe --selftest` läuft aus der Installation; die installierte App startet normal bis zum
    Einrichtungsfenster.
...
- **Live-Tests** laufen nur mit gültigem Repository-Secret `OPENROUTER_API_KEY`. Ohne Guthaben oder mit
  widerrufenem Schlüssel schlagen sie fehl (so gewollt, damit ein kaputter Schlüssel auffällt).
- **Standardmodelle** (`anthropic/claude-sonnet-5`, `~typesafe/jev-latest`) haben im Live-Test im September 2026
  geantwortet. Ob sie verfügbar bleiben, prüft der Verbindungstest in Kairo; er schlägt sonst Alternativen vor.
...
- **Browser-Erweiterung:** Sie ist nicht im Chrome Web Store bzw. bei den Edge-Add-ons veröffentlicht und muss
  entpackt geladen werden. In Firefox und Zen ist sie ohne Mozilla-Signatur nur vorübergehend installierbar
  (Signieren: [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md#firefox-add-on-signieren-für-zen-und-firefox)).
- **Zen:** Automatisiert getestet ist Firefox, also die Engine von Zen, nicht Zen selbst.
