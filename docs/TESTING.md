...
| Erweiterung | `tests/extension` (Node + Playwright, `web-ext lint`) | Windows, Linux | Content-Agent im echten Chromium und in Firefox (Gecko, die Engine von Zen), Native Messaging mit Stub-Host, Popup, Mozilla-Prüfregeln für das Firefox-Add-on |
...
| 7 | DOM-Erkennung im Browser | `tests/extension/content.test.cjs` (22 Tests im echten Chromium: Labels aller Label-Mechanismen, Sektionen, Frames, Passwortmaskierung, stabile IDs, React-kompatibles Ausfüllen, Auswahllisten und ARIA-Comboboxen, Batch), `extension.e2e.test.cjs` (echte Erweiterung + Native Messaging, inklusive Cross-Origin-iframe), `BrowserBridgeTests.*` (echter `Kairo.BrowserHost.exe` ↔ Named Pipe ↔ Kairo: Verbindungsaufbau, Request/Response, fremde Erweiterung abgewiesen, „Desktop nicht verfügbar“), `BrowserFormTests.Fills_a_web_form_through_the_browser_extension_dom_bridge`, `BrowserFormTests.Fills_a_web_form_in_firefox_through_the_extension` (echter Firefox über geckodriver: Add-on als temporäres Add-on, Host über den Mozilla-Registry-Eintrag mit Firefox-Argumenten, Formular über den DOM-Pfad ausgefüllt), `content.test.cjs` zusätzlich in Firefox, `BrowserBridgeTests.Native_host_relays_for_the_firefox_and_zen_add_on` |
...
| 13 | Fehlerbehandlung | `AgentRunnerTests.A_form_is_never_submitted_when_a_value_cannot_be_verified`, `…Values_are_verified_before_the_submit_approval`, `…Model_errors_fail_the_task_with_user_message`, `…Repeating_the_same_failing_action_is_stopped`, `…Failed_verification_triggers_targeted_correction_with_simulated_input`, `JevClientTests.Retries_transient_errors_then_succeeds`, `…Non_transient_errors_are_not_retried_and_mapped`, `OpenRouterClientTests.Falls_back_to_json_object_when_schema_not_supported`, `InfrastructureTests.Corrupted_settings_fall_back_to_defaults_and_keep_a_backup` |
| 14 | Installation und Deinstallation | `installer/test-install.ps1` in CI: stille Installation, Dateien, Verknüpfungen, Autostart, Native-Messaging-Registrierung, Selbsttest (`--selftest`, inkl. Tray-Symbole), normaler Start der installierten App bis zum Einrichtungsfenster, Deinstallation mit Behalten der Daten, Neuinstallation, Deinstallation mit sicherem Löschen der Daten |
